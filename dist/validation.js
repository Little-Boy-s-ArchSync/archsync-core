...
function findDuplicateIds(items, path) {
    const seen = new Set();
...
export function validateArchitectureSemantics(value) {
    const issues = [];
    ...
    issues.push(...findDuplicateIds(value.relationships.filter((relationship) => relationship.id !== undefined), "/relationships"), ...findDuplicateIds(value.rules ?? [], "/rules"), ...findDuplicateIds(value.quality_goals ?? [], "/quality_goals"));
...
export async function loadArchitecture(filePath) {
...
export function formatValidationIssues(issues) {
    ...
    issues.forEach((issue, index) => {
        lines.push(`${index + 1}. ${issue.message}`, `   Location: ${formatValidationPath(issue.path)}`, `   Fix: ${validationFix(issue)}`);
    });
    return lines.join("\n");
}
function formatValidationPath(path) {
    ...
    const readable = segments.reduce((current, segment) => /^\d+$/.test(segment)
        ? `${current}[${segment}]`
        : current.length === 0
            ? segment
            : `${current}.${segment}`, "");
