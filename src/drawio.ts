...
const paletteByType: Record<ComponentType, { fill: string; stroke: string }> = {
...
export function generateDrawio(
  ...
  const usedLayers = layerOrder.filter((layer) =>
    ids.some((id) => document.components[id]?.layer === layer),
  );
  const grouped = usedLayers.map((layer) =>
    ids.filter((id) => document.components[id]?.layer === layer),
  );
  ...
  for (const id of ids) {
    ...
    const detail = escapeHtmlText(
      `${component.type} · ${component.layer}${state ? ` · ${state.toUpperCase()}` : ""}`,
    );
    ...
    lines.push(
      `        <mxCell id="node-${escapeXml(id)}" value="${value}" style="${style}" vertex="1" parent="1">`,
      `          <mxGeometry x="${position.x}" y="${position.y}" width="170" height="72" as="geometry"/>`,
      "        </mxCell>",
    );
  }
    ...
    .forEach((relationship, index) => {
      ...
      if (routeAnnotatedEdge) {
        ...
        if (sourceColumnBlocked) {
          lines.push(
            `              <mxPoint x="${sourceLaneX}" y="${sourcePosition.y + 36}"/>`,
            `              <mxPoint x="${sourceLaneX}" y="${routeY}"/>`,
          );
        } else {
          lines.push(`              <mxPoint x="${sourcePosition.x + 85}" y="${routeY}"/>`);
        }
        lines.push(
          `              <mxPoint x="${targetPosition.x + 85}" y="${routeY}"/>`,
          "            </Array>",
          "          </mxGeometry>",
        );
      } else {
